...
isc.IDProvider.addClassProperties({
    ...
    getNextID: function (numPool, record, fieldName, atrNames, that, callbackSetID) {
        if (isc.IDProvider.curr <= isc.IDProvider.last) {
            isc.IDProvider.curr += 1;
            callbackSetID(record, fieldName, atrNames, that, isc.IDProvider.curr - 1);
        } else {
            isc.IDProvider.inRemote = true;
            SendCommand("IDProvider", "GET", {
                pool: numPool
            }, function (rpcResponse, data, rpcRequest) {
                if (rpcResponse.httpResponseCode == 200) {
                    var jsonObj = parseJSON(rpcResponse.httpResponseText);
                    isc.IDProvider.curr = jsonObj["numID"] + 1;
                }
                isc.IDProvider.last = isc.IDProvider.curr + numPool - 2;
                callbackSetID(record, fieldName, atrNames, that, isc.IDProvider.curr - 1);
            });
            // TODO ^^^^^^^^ Persist taken ID pool for among-session usage
        }
    },

    getCurrentID: function () {
        return isc.IDProvider.curr - 1;
    }
});

// ------ Cookie processing ------
// function saveDefaultsToCookie() 
// {
// isc.Cookie.set('InitSet',
// isc.Comm.serialize(
// [ { User : curr_User} ]) );
// }
...
// {
// var defaultsString = isc.Cookie.get('InitSet');
// if (defaultsString) {
// var fn = new Function("return " + defaultsString);
// var savedDefaults = fn();
// if (savedDefaults != null && savedDefaults.length == 1 &&
// savedDefaults[0].User != null)
// {
// curr_User = savedDefaults[0].User;
// }
// }
// }
...
isc.CBMDataSource.addProperties({
    ...
    callbackSetID: function (record, fieldName, atrNames, that, val) {
        record[fieldName] = val;
        if (typeof (record.contextForm) != 'undefined' && record.contextForm != null) {
            record.contextForm.setValue(fieldName, val);
        }
        // --- Links to Main parts from Version parts
        for (var i = 0; i < atrNames.length; i++) {
            if (that.getField(atrNames[i]).relationStructRole == "MainID" && record[atrNames[i]] == null) {
                record[atrNames[i]] = record[that.getField(atrNames[i]).mainPartID];
                if (typeof (record.contextForm) != 'undefined' && record.contextForm != null) {
                    record.contextForm.setValue(atrNames[i], record[atrNames[i]]);
                }
            }
        }
        // --- Additional cycle for child tables for this "part" ID initialization
        for (var i = 0; i < atrNames.length; i++) {
            if (that.getField(atrNames[i]).relationStructRole == "ChildID" && that.getField(atrNames[i]).part == that.getField(fieldName).part) {
                record[atrNames[i]] = val; //isc.IDProvider.getCurrentID();
                if (typeof (record.contextForm) != 'undefined' && record.contextForm != null) {
//                    record.contextForm.setValue(atrNames[i], isc.IDProvider.getCurrentID());
                    record.contextForm.setValue(atrNames[i], val);
                }
            } else if (that.getField(atrNames[i]).relationStructRole == "ID" && record[atrNames[i]] == null) {
                isc.IDProvider.getNextID(2, record, atrNames[i], atrNames, that, that.callbackSetID);
            }
        }
		// --- Additional actions after main ID setting logic.
        if(that.afterSetID) { 
			that.afterSetID(record, that);
		}
    },
    ...
    createInstance: function (contextGrid) {
        ...
        isc.IDProvider.getNextID(2, record, "ID", this.getFieldNames(false), this, this.callbackSetID);
    ...
    cloneInstance: function (srcRecord) {
        record = this.copyRecord(srcRecord);
		...
		for (ifld in this.getFields())
		{
			fld = this.getField(ifld);
	...
	copyCollection: function(fld, srcRecord, record){
		collectionRS = isc.ResultSet.create({
			...
			dataArrived: function (startRow, endRow) {
				...
				{
					rec = this.get(i); 
					dsRelated = isc.DataSource.getDataSource(fld.relatedConcept);
					...
					recNew = dsRelated.cloneInstance(rec);
				}
			}
		});
...
function editRecords(records, context) {
    // --- Find DataSource for record (or if not defined - from context)
    var ds = null;
    var recordFull = null;

    // record = {};
    // ds.constructNull(record); 
    // editRecord(record, this);


    var cls = conceptRS.findByKey(records[0]["PrgClass"]);
    if (typeof (context) != "undefined" && context != null && (typeof (cls) == "undefined" || cls == null || cls == "loading" || records.getLength() > 1)) { // DS by Context 
        ...
        if (context.dataSource != cls["SysCode"]) {
...
isc.TableWindow.addProperties({
    ...
    initWidget: function () {
        ...
        this.addItems(
        [
            // TODO Activate Filter by special button		 isc.FilterBuilder.create({ dataSource: this.dataSource, topOperator: "and" }),
            this.innerGrid
        ]);
...
function createTable(forType, context, callback, filter) {
    ...
    dsRef = table.innerGrid.grid.getDataSource();
