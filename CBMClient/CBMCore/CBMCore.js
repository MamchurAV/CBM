...
function generateDStext(forView, futherActions) {
    ...
    if (conceptRec.DataBaseStore && conceptRec.DataBaseStore !== "null") {
        resultDS +=
            'dbName: "' + conceptRec.DataBaseStore.ConnectionParams + '", ';
    }
    ...
    if (
        conceptRec.ExprToStringDetailed &&
        conceptRec.ExprToStringDetailed !== "null"
    ) {
    ...
    if (viewRec.ChildExpansionMode && viewRec.ChildExpansionMode !== "null") {
        resultDS +=
            'childExpansionMode: "' + viewRec.ChildExpansionMode + '", ';
    }
    if (viewRec.MainTabName && viewRec.MainTabName !== "null") {
        resultDS +=
            'mainTabName: "' +
            getLang(viewRec.MainTabName, tmp_Lang, false) +
            '", ';
    }
    ...
    isc.DataSource.get("PrgViewField").fetchData(
        ...
        function (dsResponce, data, dsRequest) {
            ...
            getRelationsForConcept(conceptRec.ID, function (relationsData) {
                relations = relationsData;

                // --- Just fields creation ---
                var viewFieldsCount = viewFields.getLength();
                for (var i = 0; i < viewFieldsCount; i++) {
                    var currentRelation = relations.find(
                        "ID",
                        viewFields[i].ForRelation,
                    );
                    ...
                    if (!currentRelation) {
                        resultDS +=
                            '{name: "' +
                            viewFields[i].SysCode +
                            '", editorType: "' +
                            viewFields[i].ControlType +
                            '"}, ';
                        continue;
                    }
                    ...
                    if (
                        currentRelation.ExprDefault &&
                        currentRelation.ExprDefault !== "null" &&
                        currentRelation.ExprDefault !== null
                    ) {
                        resultDS +=
                            'defaultValue: "' +
                    ...
                    if (
                        (!currentRelation.DBColumn ||
                            currentRelation.DBColumn === "null" ||
                            currentRelation.DBColumn === null ||
                            currentRelation.DBColumn === "undefined") &&
                        kind !== "CollectionControl"
                    ) {
                    ...
                    if (
                        currentRelation.Domain &&
                        currentRelation.Domain !== "null" &&
                        currentRelation.Domain !== null
                    ) {
                        resultDS +=
                            "valueMap: " + currentRelation.Domain + ", ";
                    }
                    if (
                        viewFields[i].UIPath &&
                        viewFields[i].UIPath !== "null"
                    ) {
                    ...
                    if (
                        viewFields[i].ColSpan &&
                        viewFields[i].ColSpan !== "null"
                    ) {
                        resultDS += "colSpan: " + viewFields[i].ColSpan + ", ";
                    }
                    if (
                        viewFields[i].RowSpan &&
                        viewFields[i].RowSpan !== "null"
                    ) {
                    ...
                    if (viewFields[i].Hint && viewFields[i].Hint !== "null") {
                        resultDS +=
                            'prompt: "' +
                            getLang(viewFields[i].Hint, tmp_Lang, false) +
                            '", ';
                    }
                    ...
                    if (
                        currentRelation.RelationStructRole &&
                        currentRelation.RelationStructRole !== "null" &&
                        currentRelation.RelationStructRole !== null
                    ) {
                        resultDS +=
                            'relationStructRole: "' +
                    ...
                    if (
                        currentRelation.VersPart &&
                        currentRelation.VersPart !== "null" &&
                        currentRelation.VersPart !== null
                    ) {
                        resultDS += 'part: "' + currentRelation.Part + '", ';
                    }
                    if (
                        currentRelation.MainPartID &&
                        currentRelation.MainPartID !== "null" &&
                        currentRelation.MainPartID !== null
                    ) {
                        resultDS +=
                            'mainPartID: "' +
                    ...
                    var relatedConceptRec = conceptRS.find(
                        "ID",
                        currentRelation.RelatedConcept,
                    );
                    var backLinkRelationRec = relationRS.find(
                        "ID",
                        currentRelation.BackLinkRelation,
                    );
                    // For all types of Relation - special EditorType may be defined
                    if (
                        viewFields[i].ControlType &&
                        viewFields[i].ControlType !== "null"
                    ) {
                        resultDS +=
                            'editorType: "' + viewFields[i].ControlType + '", ';
                    }
                    ...
                    if (kind === "Command") {
                        ...
                        if (
                            currentRelation.ExprFunctions &&
                            currentRelation.ExprFunctions !== "null" &&
                            currentRelation.ExprFunctions !== null
                        ) {
                            resultDS +=
                                "click: " + currentRelation.ExprFunctions;
                        }
                    } else {
                        ...
                        switch (type) {
                            ...
                            case "ShortMlString":
                                resultDS +=
                                    'type: "multiLangText", filterEditorType: "text", ';
                                if (
                                    viewFields[i].ControlType &&
                                    viewFields[i].ControlType !== "null"
                                ) {
                            ...
                            default:
                                ...
                                if (kind === "Link") {
                                    ...
                                    if (currentRelation.Root > 0) {
                                        ...
                                        currentRelation.HierarchyLink === true
                                    ) {
                                        resultDS +=
                                            'foreignKey: "' + type + '.ID", ';
                                    ...
                                    if (
                                        viewFields[i].DataSourceView &&
                                        viewFields[i].DataSourceView !== "null"
                                    ) {
                                        resultDS +=
                                            'optionDataSource: "' +
                                            viewFields[i].DataSourceView +
                                    ...
                                    if (
                                        currentRelation.LinkFilter &&
                                        currentRelation.LinkFilter !== "null"
                                    ) {
                                        resultDS +=
                                            "optionCriteria: " +
                                            currentRelation.LinkFilter +
                                    ...
                                    if (
                                        viewFields[i].ValueField &&
                                        viewFields[i].ValueField !== "null"
                                    ) {
                                        resultDS +=
                                            'valueField: "' +
                                            viewFields[i].ValueField +
                                    ...
                                    if (
                                        viewFields[i].DisplayField &&
                                        viewFields[i].DisplayField !== "null"
                                    ) {
                                        resultDS +=
                                            'displayField: "' +
                                            viewFields[i].DisplayField +
                                    ...
                                    if (
                                        viewFields[i].PickListFields &&
                                        viewFields[i].PickListFields !== null &&
                                        viewFields[i].PickListFields !== "null"
                                    ) {
                                        resultDS +=
                                            "pickListFields: " +
                                    ...
                                    if (!backLinkRelationRec) {
                                        throw new Error(
                                            "backLinkRelation not actual for field <" +
                                                fldTitle +
                                                "> of <" +
                                                forView +
                                                "> concept in generateDStext().",
                                        );
                                    }
                                    ...
                                    if (
                                        ...
                                        resultDS +=
                                            'editorType: "' +
                                            (kind === "BackAggregate"
                                                ? "CollectionAggregateControl"
                                                : "CollectionControl") +
                                            '", ';
                                    }
                                    resultDS +=
                                        'relatedConcept: "' +
                                        relatedConceptRec.SysCode +
                                        '", ';
                                    resultDS +=
                                        'backLinkRelation: "' +
                                        backLinkRelationRec.SysCode +
                                        '", ';
                                    if (
                                        viewFields[i].ValueField &&
                                        viewFields[i].ValueField !== "null"
                                    ...
                                    resultDS +=
                                        ...
                                        backLinkRelationRec.SysCode +
                                        '", ';
                                    var crossConceptRec = conceptRS.find(
                                        "ID",
                                        currentRelation.CrossConcept,
                                    );
                                    if (crossConceptRec) {
                                        resultDS +=
                                            'crossConcept: "' +
                                            crossConceptRec.SysCode +
                                            '", ';
                                    }
                                    var crossRelationRec = relationRS.find(
                                        "ID",
                ...
                if (
                    conceptRec.MenuAdditions &&
                    conceptRec.MenuAdditions !== "null"
                ) {
                    resultDS += ", MenuAdditions: " + conceptRec.MenuAdditions;
                }
                if (
                    conceptRec.CreateFromMethods &&
                    conceptRec.CreateFromMethods !== "null"
                ) {
                    resultDS +=
                        ", CreateFromMethods: " + conceptRec.CreateFromMethods;
                }
                // --- Any functions ---
...
function createDS(forView, callback) {
    ...
    generateDStext(forView, function (resultDS) {
        ...
        try {
            ...
            isc.warn(
                "Error in dynamically generearated DS " +
                    forView +
                    "." /*  Full generated text: " + resultDS*/,
            );
        }
    ...
    function processValue(innerContext) {
        ...
        if (i < valArr.length) {
            ...
            if (likeKey(tmpVal) && i < valArr.length - 1) {
                ...
                var thatConcept = conceptRS.find({
                    ID: getRelation(innerContext.Concept, valArr[i])
                        .RelatedConcept,
                });
                getObject(
                    thatConcept.SysCode,
...
function processJSONExpression(expr, context) {
    ...
    function processElement() {
        ...
        if (i < exprArr.length) {
            ...
            if (
                propVal.charAt(0) === '"' &&
                propVal.charAt(propVal.length - 1) === '"'
            ) {
                ...
                function processValue(innerContext) {
                    ...
                    if (j < valArr.length) {
                        ...
                        if (likeKey(tmpVal) && j < valArr.length - 1) {
                            // Get object for further processing
                            var thatConcept = conceptRS.find({
                                ID: getRelation(innerContext.Concept, valArr[j])
                                    .RelatedConcept,
                            });
                            getObject(
                                thatConcept.SysCode,
                        ...
                        if (j === valArr.length - 1 && i < exprArr.length) {
                            //<<< TODO: ?if expression become longer
                            outArr[i] =
                                exprArr[i].split(":")[0].trim() +
                                ':"' +
                                tmpVal +
                                '"';
                            processElement();
...
function getRelationsForConcept(conceptId, callback) {
    ...
    function innerGetRelations(conceptId, callbackBound) {
        if (conceptDS.hasAllData()) {
            ...
            isc.DataSource.get("Relation").fetchData(
                ...
                function (relationsFetched) {
                    ...
                    if (relationsToThis !== null) {
                        for (i; i < relationsToThis.length; i++) {
                            ...
                            if (!exists || !inherited) {
                                ...
                                relationsToThis[i].ForConceptSysCode =
                                    record.SysCode;
                    ...
                    if (
                        ...
                        if (record && relations) {
                            record.relations = relations.sortByProperty(
                                "Odr",
                                true,
                            );
                        }
                        if (callbackBound) {
                            callbackBound(
                                relations.sortByProperty("Odr", true),
                            );
                        }
                    }
                },
...
function getRelationsForConceptName(conceptName, callback) {
    ...
    getRelationsForConcept(conceptID, function (rel) {
...
function getRelationsForViewConcept(forView, callback) {
    ...
    if (viewRec) {
        ...
        //  if (viewRec) {
        getRelationsForConcept(viewRec.ForConcept, function (rel) {
...
var getClientOnlyDS = function (ds) {
    var dsLocal = isc.CBMDataSource.create({
        ID: ds.ID + "_" + this.ID,
        clientOnly: true,
    });
    for (var prop in ds) {
...
isc.CBMDataSource.addProperties({
    ...
    getPrgClass: function () {
        ...
        if (this.prgClass === null) {
            this.prgClass = classRS.find({
                ForConcept: this.getConcept().ID,
                Actual: true,
            });
        }
                ...
                if (
                    ...
                    for (var pairKey in dsRequest.data) {
                        if (typeof dsRequest.data[pairKey] === "string") {
                            dsRequest.data[pairKey] = dsRequest.data[
                                pairKey
                            ].replace("~|" + curr_Lang + "|", "");
                        }
                    }
                    // For fetch  - allways convert criteria to isc.AdvancedCriteria form
                    var criteria = this.convertDataSourceCriteria(
                        dsRequest.data,
                    );
                    dsRequest.data = {};
    ...
    resolveLinks: function (callback) {
        that = this;
        getRelationsForViewConcept(this.ID, function (data) {
                                ...
                                generateDStext(forView, function (resultDS) {
                                    try {
                                        ...
                                        isc.warn(
                                            "Error in dynamically generearated DS <" +
                                                forView +
                                                "> while links resolving. Error: " +
                                                err +
                                                " Full generated text: " +
                                                resultDS.substring(0, 100),
                                        );
                                    }
                                });
                            }
    ...
    setID: function (record) {
        ...
        if (
            typeof record.valuesEditor != "undefined" &&
            record.valuesEditor != null
        ) {
        ...
        for (var i = 0; i < n; i++) {
            ...
            if (
                this.getField(atrNames[i]).relationStructRole == "MainID" &&
                record[atrNames[i]] == null
            ) {
                record[atrNames[i]] =
                    record[this.getField(atrNames[i]).mainPartID];
                if (
                    typeof record.valuesEditor != "undefined" &&
                    record.valuesEditor != null
                ) {
                    record.valuesEditor.setValue(
                        atrNames[i],
                        record[atrNames[i]],
                    );
                }
            ...
            // --- Assignment for child tables for this "part" ID initialization
                ...
                this.getField(atrNames[i]).relationStructRole ==
                "ChildID" /*&& this.getField(atrNames[i]).part == that.getField(fieldName).part*/
            ) {
            ...
            // --- Assignment of other parts ID (which name in this DS is not just "ID")
                ...
                this.getField(atrNames[i]).relationStructRole == "ID" &&
                record[atrNames[i]] == null
            ) {
    ...
    setNullID: function (record) {
        ...
        for (var i = 0; i < n; i++) {
            if (
                this.getField(atrNames[i]).relationStructRole === "ID" ||
                this.getField(atrNames[i]).relationStructRole === "ChildID" ||
                this.getField(atrNames[i]).relationStructRole === "MainID"
            ) {
                record[atrNames[i]] = null;
            }
    ...
    cloneRelatedInstances: function (
        srcRecord,
        record,
        cloneNextRecord,
        afterCopyCallbacks,
        ...
        if (fieldsToCopyCollection.length > 0) {
            ...
            var recursiveCopyCollection = function () {
                ...
                if (iFld < fieldsToCopyCollection.length) {
                    if (
                        iFld == fieldsToCopyCollection.length - 1 &&
                        (thatDS.afterCopy ||
                            afterCopyCallbacks ||
                            outerCallback)
                    ) {
                        ...
                        if (thatDS.afterCopy) {
                            afterCopyCallbacks.push({
                                func: thatDS.afterCopy,
                                rec: record,
                                outerCall: outerCallback,
                            });
                        ...
                        thatDS.copyCollection(
                            fieldsToCopyCollection[iFld],
                            srcRecord,
                            record,
            ...
            // -- No structural fields - Execute afterCopy functions
            ...
            if (afterCopyCallbacks !== undefined) {
                for (var i = afterCopyCallbacks.length - 1; i >= 0; i--) {
                    afterCopyCallbacks[i].func(
                        afterCopyCallbacks[i].rec,
                        afterCopyCallbacks[i].outerCall,
                    );
                }
    ...
    copyCollection: function (
        ...
        isc.DataSource.get(fld.relatedConcept).fetchData(
            parseJSON(
                '{"' +
                    fld.backLinkRelation +
                    '" : "' +
                    srcRecord[fld.mainIDProperty] +
                    '", "Del": false }',
            ),
            function (dsResponce, data, dsRequest) {
                if (data.length === 0) {
                    ...
                    function cloneNextRecord() {
                        ...
                        if (z < data.length) {
                            var rec = data[z];
                            var dsRelated = isc.DataSource.getDataSource(
                                fld.relatedConcept,
                            ); // Instead of closures - define very time here
                            if (z < data.length - 1) {
                                recNew = dsRelated.cloneMainInstance(
                                    rec,
                                    cloneNextRecord,
                                );
                                recNew[fld.backLinkRelation] = record["ID"];
                                function cloneRecordRelatedInstances() {
                                    dsRelated.cloneRelatedInstances(
                                        rec,
                                        recNew,
                                        cloneNextRecord,
                                    );
                                ...
                                TransactionManager.add(
                                    recNew,
                                    recNew.currentTransaction,
                                );
                                cloneRecordRelatedInstances();
                            } else {
                                // The last record - callbacks and post-actions provided
                                ...
                                function cloneLastRecordRelatedInstances() {
                                    dsRelated.cloneRelatedInstances(
                                        ...
                                        cloneNextRecord,
                                        callbacks,
                                    ); // The last row only - processed with callbacks
                                    if (recursiveCopyCollection) {
                                        recursiveCopyCollection();
                                    }
                                    if (cloneNextRecordPrev) {
    ...
    prepareFormLayout: function (valuesManager, record, context) {
        ...
        var atrNames = this.getFieldNames(false);
        var mainUIPath = this.mainTabName
            ? this.mainTabName
            : isc.CBMStrings.FormWindow_MainTab;
        ...
        for (var i = 0; i < n; i++) {
            if (
                typeof this.getField(atrNames[i]).hidden == "undefined" ||
                this.getField(atrNames[i]).hidden !== true ||
                ...
                if (!that.getField(atrNames[i])) {
                    ...
                    if (
                        !currRoot ||
                        typeof currRoot === "undefined" ||
                        currRoot === null ||
                        typeof currRoot === "null"
                    ) {
                        currRoot = mainUIPath;
                    }
                }
                ...
                for (; j < UIPaths.length; j++) {
                    if (UIPaths[j] === currRoot) {
                        ...
                        if (this.getField(atrNames[i]).kind != "Command") {
                            items[j][nItem] = isc.FormItem.create({
                                name: atrNames[i],
                                width: "100%",
                                hidden: null,
                                showIf: null,
                        ...
                        if (this.getField(atrNames[i]).defaultValue) {
                            try {
                                items[j][nItem].defaultValue = eval(
                                    this.getField(atrNames[i]).defaultValue,
                                );
                            } catch (e) {
                                if (
        ...
        for (var i = 0; i < UIPaths.length; i++) {
            ...
            var height = 100;
            height =
                items[i].getLength() * 16 -
                (items[i].getLength() * items[i].getLength()) / 16;
            if (height > hiHeight) {
                hiHeight = height;
            }
            ...
            buttons: [
                {
                    ...
                    click: function () {
                        ...
                        var currentValue =
                            this.topElement.valuesManager.values.Description;
                ...
                },
                {
                    ...
                    click: function () {
                        ...
                        if (
                            this.topElement.contextObject !== null && // <<< Can be null if window opend for multi-records editing
                            this.topElement.contextObject.currentTransaction !==
                                null &&
                            this.topElement.contextObject.currentTransaction
                                .Changes.length > 0
                        ) {
                            var that = this;
                            isc.confirm(
                                isc.CBMStrings.CancelButton_SaveOrNot,
                                /*             function (ok) {
                if (ok) {
    ...
    edit: function (record, context) {
        ...
        var form = isc.FormWindow.create({
            ...
            save: function (topCancel) {
                if (this.valuesManager.validate(true)) {
                    ...
                    isc.DataSource.get(this.dataSource).onFormSave(
                        this.valuesManager.getValues(),
                        this,
                        context,
                    );
                                ...
                                if (
                                    ...
                                    (values[attr] !== undefined &&
                                        ((values[attr] === null &&
                                            ...
                                            (!(values[attr] instanceof Date) &&
                                                values[attr] !==
                                                    oldValues[attr]) ||
                                            // For Date - special checks
                                            (values[attr] instanceof Date &&
                                                values[attr].getTime() !==
                                                    oldValues[attr].getTime())))
                                ) {
                    ...
                    if (
                        ...
                        context.parentElement.parentElement.canvasItem.showValue(
                            "",
                            null,
                            context.parentElement.parentElement.parentElement,
                            context.parentElement.parentElement.canvasItem,
        ...
        if (this.contextObject && this.contextObject.Description) {
            form.title =
                form.title +
                " <" +
                this.contextObject.Description +
                "> - (" +
                stateTitle +
                ")";
        } else {
            ...
            save: function () {
                ...
                for (var j = 0; j < records.getLength(); j++) {
                    ...
                    for (var i = 0; i < atrNames.length; i++) {
                        if (
                            valuesManager.values[atrNames[i]] != null &&
                            valuesManager.values[atrNames[i]] != "ID"
                        ) {
                            records[j][atrNames[i]] =
                                valuesManager.values[atrNames[i]];
...
var CBMobject = {
    ...
    save: function (real, context, contextField, callback) {
        ...
        if (this.infoState === "new" || this.infoState === "copy") {
            ...
            if (
                this.ds.getCacheData() &&
                this.ds.getCacheData().find({ ID: this.ID })
            ) {
            ...
            if (real) {
                this.ds.addData(this.getPersistentChanged(), function () {
                    if (
                        that.ds.getCacheData() &&
                        !that.ds.getCacheData().find({ ID: that.ID })
                    ) {
                        addDataToCache(that);
                    }
                });
            ...
            if (real) {
                var that = this;
                this.ds.updateData(this.getPersistentChanged(), function () {
                    //Quick update
   ...
   */
    ...
    testMainRecordSaved: function (
        additionalMsg,
        contextValuesManager,
        callback,
        args,
    ) {
        var that = this;
        if (this.infoState === "new") {
            isc.confirm(
                isc.CBMStrings.Object_MainRecordNotSaved +
                    (additionalMsg ? additionalMsg : ""),
                function (ok) {
                    if (ok) {
                        that.save(true, null, null, function () {
                            if (callback) {
                                callback(args);
                            }
                        });
                        if (contextValuesManager) {
                            contextValuesManager.setValue(
                                "infoState",
                                "loaded",
                            );
                        }
...
function createFromRecord(srcRecord) {
    if (srcRecord.Concept) {
        ...
        var atrNames = isc.DataSource.get(srcRecord.Concept).getFieldNames(
            false,
        );
        var n = atrNames.length;
        for (var i = 0; i < n; i++) {
...
function editRecords(records, context, conceptRecord, trans) {
    ...
    if (
        records[0].currentTransaction === undefined ||
        records[0].currentTransaction === null
    ) {
        if (!trans) {
            if (
                context &&
                context.topElement &&
                context.topElement.contextObject &&
    ...
    if (
    ...
    } else if (records.getLength() === 1) {
        // DS by exact record Class
        ...
        if (
            context.dataSource != cls["SysCode"] &&
            records[0]["infoState"] == "loaded"
        ) {
...
function createFrom(srcRecords, resultClass, initFunc, context) {
    if (srcRecords === null) {
        isc.warn(
            isc.CBMStrings.ListCreateFrom_NoSelectionDone,
            this.innerCloseNoChoiceDlg,
        );
        return;
    }
    ...
    initDestRecord = function (record) {
        ...
        if (initFunc) {
            if (mainConcept) {
                ...
                initFunc(record, srcRecords[iteration], function (record) {
                    if (context) {
    ...
    newRecord = function () {
        if (resultClass) {
            var newRec = isc.DataSource.getDataSource(
                resultClass(srcRecords[iteration]),
            ).createInstance();
        } else if (context) {
...
function deleteRecord(record, delMode, contextGrid, mainToBin) {
    ...
    var deleteLinkedRecord = function (fld, record, delMode, mainToBin) {
        ...
        if (!dsInner) {
            isc.warn(
                isc.CBMStrings.NoDataSourceDefined +
                    isc.CBMStrings.MD_ForViewField +
                    fld.name +
                    " (in function deleteLinkedRecord(). It's most likely You set flag CopyLinked for Attribute with no need.)",
            );
            return;
        }
        dsInner.fetchRecord(
    ...
    for (var i = 0; i < atrNames.length; i++) {
        ...
        if (
            (fld.editorType == "CollectionControl" ||
                fld.editorType == "CollectionAggregateControl") &&
            fld.deleteLinked == true
        ) {
    ...
    if (delMode == "deleteForce") {
        ...
        // delMode != "deleteForce" - process depending on "Del" flag existence
        if (record.Del != undefined) {
            // "Del" flag exists
            if (delMode == "delete") {
                record.Del = true;
            } else {
                // delMode == "restore" remains
                record.Del = false;
            }
...
function getLang(value, language, strictLang) {
    ...
    if (i !== -1) {
        ...
        i = tmp.indexOf("~|", 1);
        if (i !== -1) {
            // The substring is not the last
            out = tmp.substring(0, i);
        } else {
            out = tmp;
        }
    } else if (!strictLang) {
        // No requested locale - get first.
        // TODO: First part - temporary. Change to get next in language substitution list.
        i = value.indexOf("~|", 1);
        if (i != -1) {
            // Some successor locale exists
            out = value.substring(0, i);
        }
    } else {
        // Strict language condition - no default allowed
        return null;
    }
    ...
    parseInput: function (value, field, form, record) {
        ...
        if (field && field.itemLang) {
            ...
            if (i !== -1) {
                ...
                if (i === 0) {
                    // Preserve language prefix of first part in all cases
    ...
    getFilter: function (form, item, value) {
        ...
        if (relMetadata.LinkFilter && relMetadata.LinkFilter !== "null") {
            if (!relMetadata.LinkFilter.startsWith("function")) {
                ...
                var filterStr = processJSONExpression(
                    relMetadata.LinkFilter,
                    this.form.valuesManager.values,
                );
...
function deleteSelectedRecords(innerGrid, mode) {
    ...
    isc.confirm(isc.CBMStrings.InnerGridMenu_DeletionPrompt, function (ok) {
        if (ok) {
            ...
            for (var i = 0; i < n; i++) {
                ...
                if (innerGrid.context && innerGrid.context.valuesManager) {
                    cbmRecord.currentTransaction =
                        innerGrid.context.valuesManager.getValues().currentTransaction;
                }
...
var defaultContextMenuData = [
    ...
    {
        click: function () {
            var deleteMode = this.context.parentElement.parentElement.context
                ? "delete"
                : "deleteForce";
...
var innerGridContextMenu = isc.Menu.create({
    ...
    setContext: function (cont) {
        this.context = cont;
        defaultContextMenuData[0].title =
            isc.CBMStrings.InnerGridMenu_CreateNew;
        ...
        defaultContextMenuData[4].dynamicTitle =
            "this.context.getDataSource().isDeleteToBin() ? isc.CBMStrings.InnerGridMenu_DeleteToBin : isc.CBMStrings.InnerGridMenu_Delete";
        defaultContextMenuData[4].dynamicIcon =
            'this.context.getDataSource().isDeleteToBin() ? isc.Page.getAppImgDir() + "bin.png" : isc.Page.getAppImgDir() + "delete.png"';
        if (typeof cont.getDataSource().MenuAdditions != "undefined") {
            this.setData(
                defaultContextMenuData.concat(
                    cont.getDataSource().MenuAdditions,
                ),
            );
        } else {
...
isc.InnerGrid.addProperties({
        ...
        this.continueInitInnerGrid = function () {
            ...
            for (var i = 0; i < dsflds.length; i++) {
                ...
                if (typeof fld.rootValue != "undefined") {
                    fld.rootValue =
                        typeof that.treeRoot == "undefined"
                            ? fld.rootValue
                            : that.treeRoot;
                }
            }
                    ...
                    dataArrived: function () {
                        ...
                        if (!this.notFirstDataArived) {
                            if (
                                ...
                                that.ID.startsWith("isc_InnerGrid_")
                            ) {
                                that.setListSettings();
                            } else {
                                if (
            ...
            that.grid.callObjectsEdit = function (mode) {
                ...
                var viewRecord = viewRS.find(
                    "SysCode",
                    this.dataSource.ID ? this.dataSource.ID : this.dataSource,
                );
                ...
                if (mode == "new") {
                    ...
                    if (isSuper && !viewRecord["StrictConcept"]) {
                        ...
                        var newChild = function (record) {
                            var dsNew = isc.DataSource.getDataSource(
                                record[0].SysCode,
                            );
                            if (dsNew == null) {
                            ...
                            var conceptRecord = conceptRS.find(
                                "SysCode",
                                dsNew.ID,
                            );
                        ...
                        records[0]["infoState"] = "new";
                        // If hierarchy - set parent value as in selected record (if any selected)
                        var hierarchyLink = ds.findRelation({
                            HierarchyLink: true,
                        }).SysCode;
                        if (hierarchyLink && this.getSelection().length > 0) {
                            records[0][hierarchyLink] =
                                this.getSelection()[0][hierarchyLink];
                        }
                        // --- Set fields partisipating in criteria to criteria value ---
                        var criter = this.getCriteria();
                        for (var fld in criter) {
                ...
                toContextReturnButton = isc.IconButton.create({
                    ...
                    click: function () {
                        ...
                        if (thisInnerGrid.grid.anySelected()) {
                            this.topElement.callback(
                                thisInnerGrid.grid
                                    .getDataSource()
                                    .copyRecords(
                                        thisInnerGrid.grid.getSelectedRecords(),
                                    ),
                            ...
                            isc.warn(
                                isc.CBMStrings.InnerGrid_NoSelectionDone,
                                this.innerCloseNoChoiceDlg,
                            );
                        }
                        return false;
            ...
            var innerGridDefaultMenu = [
                ...
                isc.IconButton.create({
                    ...
                    click: function () {
                        this.parentElement.parentElement.parentElement.grid.callObjectsEdit(
                            "new",
                        );
                        return false;
                    },
                ...
                isc.IconMenuButton.create({
                    ...
                    visibility: that.getDataSource().isDeleteToBin()
                        ? "inherit"
                        : "hidden",
                        ...
                        data: [
                            {
                                ...
                                title: isc.CBMStrings
                                    .InnerGrid_ProcessBinSubMenu,
                                click: function (context) {
                                    context.currentInnerGrid.addFilter(
                                        "Del",
                                        { Del: true },
                                        true,
                                    );
                                    ...
                                    if (
                                        ...
                                        context.currentInnerGrid.canvasItem.showValue(
                                            "",
                                            null,
                                    ...
                                    context.currentInnerGrid.grid.contextMenu =
                                        innerGridBinContextMenu;
                                    context.currentInnerGrid.grid.showContextMenu =
                                        function () {
                                            innerGridBinContextMenu.setContext(
                                                this,
                                            );
                                            return innerGridBinContextMenu.showContextMenu();
                                        };
                                    context.currentInnerGrid.menuContainer.setMembers(
                                        innerGridBinMenu,
                                    );
                                    return false;
                                },
                ...
                isc.IconButton.create({
                    ...
                    hoverWidth: 130,
                    click: function () {
                        var deleteMode = this.parentElement.parentElement
                ...
                isc.IconButton.create({
                    ...
                    click: function () {
                        // TODO: below - sample only!!!
                        //grid.filterData(advancedFilter.getCriteria());
                        var thisGrid = this.parentElement.parentElement;
                        thisGrid.parentElement.isFilterVisible =
                            !thisGrid.parentElement.isFilterVisible;
                        thisGrid.members[1].setShowFilterEditor(
                            thisGrid.parentElement.isFilterVisible,
                        );
                        return false;
                    },
            ...
            var innerGridBinMenu = [
                isc.IconButton.create({
                    ...
                    currentInnerGrid: that,
                    click: function (context) {
                        this.parentElement.parentElement.parentElement.addFilter(
                            "Del",
                            { Del: false },
                            true,
                        );
                        ...
                        if (
                            ...
                            this.parentElement.parentElement.parentElement.canvasItem.showValue(
                                "",
                                null,
                                this.parentElement.parentElement.parentElement
                                    .topElement,
                        ...
                        this.parentElement.parentElement.parentElement.grid.contextMenu =
                            innerGridContextMenu;
                        this.parentElement.parentElement.parentElement.grid.showContextMenu =
                            function () {
                                innerGridContextMenu.setContext(this);
                                return innerGridContextMenu.showContextMenu();
                            };
                        this.parentElement.parentElement.parentElement.menuContainer.setMembers(
                            innerGridDefaultMenu,
                        );
                        return false;
                    },
                }),
    ...
    setListSettings: function () {
        if (this.topElement) {
            // Settings for top-level List window
            ...
            // Settings for in-form collection controls
            this.listSettings = listSettingsRS.find({
                ForUser: curr_User,
                ForType: this.grid.dataSource,
        ...
        if (
            typeof this.listSettings == "undefined" ||
            ...
            this.listSettings.Win = this.topElement
                ? this.topElement.getClassName()
                : this.grid.topElement.getClassName();
            this.listSettings.ForUser = curr_User;
            this.listSettings.Context = this.topElement
                ? this.topElement.contextString
                : this.grid.getDataSource().ID;
...
isc.CollectionControl.addProperties({
    ...
    showValue: function (displayValue, dataValue, form, item) {
        ...
        if (dataValue && dataValue.length > 0) {
            ...
            // Data not supplied - get it from Storage
            if (
                typeof form.valuesManager != "undefined" &&
                form.valuesManager != null
            ) {
                this.mainID = form.valuesManager.getValue(this.mainIDProperty);
                if (typeof this.mainID != "undefined") {
                    var filterString =
                        '{"' +
                        this.backLinkRelation +
                        '" : "' +
                        this.mainID +
                        '"}';
                    this.innerGrid.addFilter(
                        "BackLink",
                        parseJSON(filterString),
                        true,
                    );
...
isc.ClassFactory.defineClass(
    "RelationsAggregateControl",
    isc.CollectionAggregateControl,
);
...
isc.WeekWorkControl.addProperties({
    ...
    showValue: function (displayValue, dataValue, form, item) {
        ...
        if (typeof this.mainID != "undefined") {
            var filterString =
                '{"' + this.backLinkRelation + '" : "' + this.mainID + '"}';
        ...
        for (var i = 1; i < 8; i++) {
            ...
            if ((beg || end) && !record) {
                ...
                (new Date(record.OpeningTime).getHours() !== beg.getHours() ||
                    new Date(record.OpeningTime).getMinutes() !==
                        beg.getMinutes() ||
                    new Date(record.ClosingTime).getHours() !==
                        end.getHours() ||
                    new Date(record.ClosingTime).getMinutes() !==
                        end.getMinutes())
            ) {
                // Update
                var cbmRecord = Object.create(CBMobject);
...
isc.TableWindow.addProperties({
    ...
    initWidget: function () {
        ...
        this.title =
            (titleDS ? titleDS : this.dataSource) +
            isc.CBMStrings.TableWindow_Title;
    ...
    onCloseClick: function () {
        ...
        if (this.innerGrid.listSettingsChanged) {
            this.innerGrid.listSettings.Settings =
                this.innerGrid.grid.getViewState();
            if (this.innerGrid.listSettingsExists) {
                listSettingsRS.dataSource.updateData(
                    this.innerGrid.listSettings,
                );
            } else {
    ...
    testCreateDS(forType, function () {
        var initCreatedTable = function (table) {
            ...
            if (filter !== undefined && filter !== null && context != table) {
                filter = isc.DataSource.combineCriteria(
                    filter,
                    table.innerGrid.grid.getCriteria(),
                );
            ...
            table.innerGrid.grid.fetchData(
                filter,
                function (dsResponse, data, dsRequest) {
...
isc.FormWindow.addProperties({
    ...
    saveGridsInItems: function (items) {
        for (var i = 0; i < items.length; i++) {
            var item = items[i];
            if (
                /*item.kind === "BackAggregate" || item.kind === "BackLink" || item.kind === "CrossLink"
          || */ item.editorType === "CollectionAggregateControl" ||
                item.editorType === "CollectionControl" ||
                item.editorType === "CollectionCrossControl" ||
                item.editorType === "RelationsAggregateControl"
            ) {
                ...
                if (item.innerGrid.listSettingsChanged) {
                    ...
                    item.innerGrid.listSettings.Settings =
                        item.innerGrid.grid.getViewState();
                    if (item.innerGrid.listSettingsExists) {
                        listSettingsRS.dataSource.updateData(
                            item.innerGrid.listSettings,
                        );
                    } else {
                        listSettingsRS.dataSource.addData(
                            item.innerGrid.listSettings,
                        );
                    }
                }
...
isc.LeafletCanvas.addProperties({
    ...
    getInnerHTML: function () {
        var divHtml =
            "<div id=leaflet_" +
            this.ID +
            ' style="height: auto; min-height: 200px; width: auto;">Place for Map</div>';
        return divHtml;
    },
    ...
    draw: function () {
        ...
        L.tileLayer(
            "https://api.tiles.mapbox.com/v4/{id}/{z}/{x}/{y}.png?access_token={accessToken}",
            {
                ...
                maxZoom: 21,
                id: "mapbox.streets",
                accessToken: "<Leaflet_token_here>" // <<< TODO - provide external token here
            },
        ).addTo(this.mymap);
    ...
    zoomEnd: function () {
        if (this.dataRefresh) {
            this.dataRefresh("zoom");
        }
    },

    moveEnd: function () {
        if (this.dataRefresh) {
            this.dataRefresh("move");
        }
    },
...
isc.LeafletControl.addProperties({
    ...
    setValue: function (val) {
        if (val) {
            ...
            // <<< to prevent second initialization
            ...
            if (latFld && lngFld) {
                ...
                if (lat && lng) {
                    ...
                    if (adrFld && adrFld.getValue()) {
                        ...
                        this.canvas.marker = L.marker([lat, lng])
                            .addTo(this.canvas.mymap)
                            .bindPopup(adr)
                            .openPopup();
                    } else {
                        this.canvas.marker = L.marker([lat, lng]).addTo(
                            this.canvas.mymap,
                        );
                    }
                } else {
...
function directGeocodingYandex(address, callback) {
    isc.RPCManager.sendRequest({
        ...
        callback: function (RPCResponse) {
            ...
            for (
                var i = 0;
                i < resp.response.GeoObjectCollection.featureMember.length;
                i++
            ) {
                var coordsAdr =
                    resp.response.GeoObjectCollection.featureMember[i].GeoObject
...
function getGeocodingResults(address, form, callback) {
    ...
    directGeocodingYandex(address, function (results) {
...
function fillGeocoding(form, item) {
    ...
    if (adrFld) {
        getGeocodingResults(adrFld.getValue(), form, function (result) {
