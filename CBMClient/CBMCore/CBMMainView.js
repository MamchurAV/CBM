...
var loadCommonData = function () {
    ...
    // Conceptual metadata
    isc.DataSource.get("Concept").fetchData(
        null,
        function (dsResponce, data, dsRequest) {
            ...
            isc.DataSource.get("Relation").fetchData(
                ...
                function (dsResponce, data, dsRequest) {
                    ...
                    isc.DataSource.get("RelationKind").fetchData(
                        null,
                        function (dsResponce, data, dsRequest) {
                            ...
                            isc.DataSource.get("PrgView").fetchData(
                                null,
                                function (dsResponce, data, dsRequest) {
...
isc.Window.create({
            ...
            fields: [
                {
                    ...
                    hoverWidth: "170",
                    changed: function () {
                        this.form.topElement.changeLoginWindowLocale(
                            this.getValue(),
                        );
                        this.form.redraw();
                    },
                ...
                },
                {
                    ...
                    click: function () {
                        if (!this.form.items[4].isVisible()) {
                            ...
                            this.button.setIcon(
                                isc.Page.getAppImgDir() +
                                    "arrow_right_long.png",
                            );
                            this.setTitle("");
                            this.disable();
                        }
    ...
    changeLoginWindowLocale: function (loc) {
        if (loc === "ru-RU") {
            ...
            this.items[1].getFields()[0].prompt =
                "Выберите язык и национальные настройки при работе в системе";
            this.items[1].getFields()[2].title = "Системное имя (логин)";
            this.items[1].getFields()[2].prompt =
                "Условное имя для работы в системе";
            this.items[1].getFields()[3].title = "Пароль";
            this.items[1].getFields()[3].prompt =
                "Введите пароль для работы в CBM";
            this.items[1].getFields()[4].title = "Подтвердите пароль";
            this.items[1].getFields()[4].prompt =
                "При первоначальной регистрации, во избежание ошибок, повторно введите пароль";
            this.items[1].getFields()[5].title = "Работать с БД";
            this.items[1].getFields()[5].prompt =
                "Выберите экземпляр системы, с которым будете работать";
            this.items[1].getFields()[6].setTitle("Регистрация");
            this.items[1]
                .getFields()[6]
                .setPrompt(
                    'Если Вы еще не работали в системе - нажмите, появится поле для подтверждения пароля, после чего войдите нажав "Начать работу".',
                );
            ...
            this.items[1].getFields()[4].prompt =
                "Confirm password for first-time registration";
            this.items[1].getFields()[5].title = "System Instance";
            this.items[1].getFields()[5].prompt =
                "Choose CBM instance You want to work with";
            this.items[1].getFields()[6].setTitle("Register");
            this.items[1]
                .getFields()[6]
                .setPrompt(
                    "Press if You are new CBM user, to register yourself in the system",
                );
            this.items[1].getFields()[7].setTitle("Enter Program");
            this.items[1]
                .getFields()[7]
                .setPrompt("Press to start work in CBM");
        } else {
            this.setTitle("Welcome to CBM-based system");
            this.items[1].getFields()[0].title = "Your location";
            this.items[1].getFields()[0].prompt =
...
var loginClose = function () {
    if (loginWindow.items[1].getFields()[4].visible) {
        if (
            loginWindow.items[1].getFields()[4].getValue() !==
            loginWindow.items[1].getFields()[3].getValue()
        ) {
    ...
    curr_User = loginWindow.items[1].getFields()[2].getValue().trim();
    ...
    scriptCBM.src =
        "CBMClient/locales/strings_" + curr_Lang.substr(0, 2) + ".properties";
    ...
    if (curr_Lang !== "en-GB") {
        ...
        script.src =
            "CBMClient/isomorphic/locales/frameworkMessages_" +
            curr_Lang +
            ".properties";
    ...
    mainControlPanel
        .getFields()[0]
        .setValue(curr_Date.local().format(dateTimeFormat));

    if (
        typeof curr_User == "undefined" ||
        curr_User == null ||
        curr_User == ""
    ) {
        //	 isc.warn("Sory, but Login is mandatory. \r\n Enter Your login please...");
        isc.warn(
            "Вы не ввели логин. Его наличие обязательно. \r\n Введите пожалуйста логин...",
        );
    } else if (
        typeof curr_Img == "undefined" ||
        curr_Img == null ||
        curr_Img == ""
    ) {
        //		isc.warn("Sory, but Password is mandatory. \r\n Enter Your password please...");
        isc.warn(
            "Вы не ввели пароль. Его наличие обязательно. \r\n Введите пожалуйста пароль...",
        );
        ...
        isc.warn(
            "Извините, но логин - пароль не прошли проверку. В одном из них ошибка.",
        );
    }
    return false;
};
...
var setUser = function () {
    var set_curr_User = B64.decode(curr_User).replace(/\0+$/, "");
    var set_curr_Img = B64.decode(curr_Img).replace(/\0+$/, "");
    var strCookie = "";
    if (typeof set_curr_User != "undefined" && set_curr_User != null) {
        strCookie = isc.Cookie.get("ImgFirst");
        if (
            typeof strCookie == "undefined" ||
            strCookie == null ||
            strCookie == ""
        ) {
            if (isc.Offline.get("lastCookie") == null) {
                ...
                isc.warn(
                    'Проблема с "куки". Проверьте что "куки" разрешены в Вашем броузере. Обратитесь за помощью к администратору',
                );
                return false;
        ...
        //    var jsonArr = parseJSON("[" + strCookie + "]"); // <<< НЕпонятно зачем это было
        //    var jsonCookie = eval("(" + jsonArr[0] + ")");
        var jsonCookie = parseJSON(strCookie);
        if (typeof jsonCookie != "undefined" && jsonCookie != null) {
            var tmpImg = UUID.generate();
            var rsa = new RSAKey();
            rsa.setPublic(jsonCookie.n, jsonCookie.e);
            var txt1 = rsa.encrypt(
                set_curr_User + ",img:" + set_curr_Img + ",tmp:" + tmpImg,
            );
        }
        var txt = B64.encode(
            "k:" +
        ...
        curr_Img = "0"; //Switch curr_Img to some other purpouse - server hits counter
...
isc.HLayout.create({
    ...
    members: [
        isc.SectionStack.create({
            ...
            sections: [
                ...
                {
                    title: "Navigation",
                    sectionExpanded: false /*, items:[windowsOpened] */,
                },
                {
                    title: "Instructions",
    ...
    show: function () {
        this.members[0].sections[0].setProperty(
            "title",
            isc.CBMStrings.MainMenu_Title,
        );
        this.members[0].sections[0].controls[0]
            .getFields()[0]
            .setProperty("title", isc.CBMStrings.MainMenu_DateTime);
        this.members[0].sections[1].setProperty(
            "title",
            isc.CBMStrings.MainMenu_OpendNavigation,
        );
        this.members[0].sections[2].setProperty(
            "title",
            isc.CBMStrings.MainMenu_Support,
        );
        this.Super("show");
    },
});
