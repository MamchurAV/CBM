...
else isc._preLog=[isc._pTM]}isc.definingFramework=true;if(window.isc&&isc.version!="v14.1p_2026-08-14/LGPL Development Only"&&!isc.DevUtil){isc.logWarn("SmartClient module version mismatch detected: This application is loading the core module from SmartClient version '"+isc.version+"' and additional modules from 'v14.1p_2026-08-14/LGPL Development Only'. Mixing resources from different SmartClient packages is not supported and may lead to unpredictable behavior. If you are deploying resources from a single package you may need to clear your browser cache, or restart your browser."
...
);isc.B._maxIndex=isc.C+20;isc.ClassFactory.defineClass("ScrollStick","Button");isc.A=isc.ScrollStick.getPrototype();isc.A.width=20;isc.A.title="O";isc.A.deadAreaSize=10;isc.A.sensitivity=1.0;isc.A=isc.ScrollStick.getPrototype();isc.B=isc._allFuncs;isc.C=isc.B._maxIndex;isc.D=isc._funcClasses;isc.D[isc.C]=isc.A.Class;isc.B.push(isc.A.mouseStillDown=function isc_ScrollStick_mouseStillDown(){var _1=isc.EH.getLastEvent(),_2=(_1.x-this.getPageLeft()-(this.getVisibleWidth()/2)),_3=(_1.y-this.getPageTop()-(this.getVisibleHeight()/2));if(Math.abs(_2)<this.deadAreaSize)_2=0;if(Math.abs(_3)<this.deadAreaSize)_3=0;_2*=this.sensitivity;_3*=this.sensitivity;var _4=this.target.getScrollLeft()+_2,_5=this.target.getScrollTop()+_3;this.target.scrollTo(_4,_5)}
);isc.B._maxIndex=isc.C+1;isc.$6y={invertClosedGripIfTargetAfter:true,canDrag:true,dragAppearance:"none",dragStartDistance:1,canCollapse:true,canCollapseOnTap:true,cursor:"hand",vResizeCursor:"row-resize",hResizeCursor:"col-resize",resizeInRealTime:false,$jp:false,$jo:false,overflow:"hidden",isMouseTransparent:true};isc.$6z={initWidget:function(){this.setVertical(this.vertical);this.Super("initWidget",arguments);if(isc.Browser.isMoz)this.bringToFront()},setVertical:function(_1,_2){this.vertical=_1;if(_2){this.setSrc(_1?this.vSrc:this.hSrc);if(this.showGrip)this.$207g()}
