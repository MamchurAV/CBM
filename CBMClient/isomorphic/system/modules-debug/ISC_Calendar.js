...
// +link{timelineView, timeline}.  The user can navigate using back and forward buttons or
...
// +link{addEventButton, Add Event} button.  In the day, week and timeline views, the user may
...
getMinutePixels : function (minutes, rowSize, view) {
    var view = view || this.getSelectedView();
...
scrollToTime : function (time) {
    ...
    if (isc.isA.Date(time)) {
        var sRow = this.getRowFromDate(grid, time);
...
// only works for timelineView for now
...
//> @object Lane
// Lane shown in a +link{Timeline} view, or in a +link{dayView, day view} when
// +link{showDayLanes} is true.  Each lane is a row or column, respectively, that can contain a
...
// displayed in a +link{dayView} as a result of +link{showDayLanes} being set.
//
// @visibility external
//<
...
processSaveResponse : function (dsResponse, data, dsRequest, oldEvent) {
            ...
            (isUpdate && this._shouldRefreshDay(oldStart, oldEnd)))
    {
        if (!this.dayViewSelected()) this.dayView._needsRefresh = true;
            ...
            if (isUpdate) {
                ...
                this.dayView.refreshEvents();
            } else if (isAdd) this.dayView.addEvent(newEvent, true);
        }
    }
    if (this._shouldRefreshWeek(startDate, endDate)) {
        if (!this.weekViewSelected()) this.weekView._needsRefresh = true;
            ...
            } else if (isAdd) this.weekView.addEvent(newEvent, true);
        }
    }
...
_getNewEventWindow : function (event, view) {
    ...
    if (eventWin) {
        eventWin.setProperties(props);
        eventWin.setStyleName(styleName);
     } else {
        // create eventWindow as an autoChild so it can be customized.
        var eventWin = this.createAutoChild("eventWindow", props, isc.EventWindow);
    }

    return eventWin;
},

_getEventsInRange : function (start, end, grid) {
        ...
        for (var i = 0; i < dataLength; i++) {
            ...
            {
                if (grid && grid._isWeek) results.add(curr);
...
_getEventsTouchingRange : function (start, end, recalcRange, skipEvents, lane) {
...
// need redrawing when their overlapped events redraw.
    ...
    while (!finished) {
        for (var i = 0; i < dataLength; i++) {
            ...
            if (!grid._isWeek && this.showDayLanes && lane && curr[this.laneNameField] != lane) continue;
...
_findEventWindow : function (event, grid) {
    // return the eventWindow object containing the passed event
    grid = grid || this.getSelectedView();
    var isWeek = grid.isWeekView();
    ...
    //var win = this._getNewEventWindow
    return false;
},
...
getCellDate : function (rowNum, colNum) {
    var retDate;
    if (this.dayViewSelected()) {
        retDate = this.dayView.getCellDate(rowNum, colNum);
    } else if (this.weekViewSelected()) {
        retDate = this.weekView.getCellDate(rowNum,
                    colNum - (this.weekView.frozenFields ? this.weekView.frozenFields.length : 0));
    } else if (this.monthViewSelected()) {
...
gridProps: {
    ...
    eventDragTargetDefaults: {
        ...
        positionToEventWin : function (show) {
            ...
            if (this.fillOverlapSlots) {
                ...
                if (view.isTimelineView()) {
                    ...
                    var col = view.getEventColumn(left);
                    left = view.getColumnLeft(col);
                    width = view.getColumnWidth(col);
                }
            }
            ...
            for (var i=0; i<dataLen; i++) {
                ...
                if (overlappers && overlappers.length > 0) {
                    ...
                    for (var k=0; k<tR.length; k++) {
                        if (range[cal.laneNameField] != tR[k][cal.laneNameField]) continue;
                        var overlaps = this.eventsOverlap(range, tR[k], true);
                        if (overlaps) {
                            // this range overlaps another range
                            if (range.totalSlots > tR[k].totalSlots) {
                                event._overlapProps.totalSlots = range.totalSlots;
                            ...
                            this.mergeOverlapRanges(range, tR[k]);
                            if (!touchedRanges.contains(tR[k])) touchedRanges.add(tR[k]);
                            addRange = false;
                        }
                        if (!addRange) break;
                    }
                ...
                if (addRange) {
                    ranges.add(range);
                    if (!touchedRanges.contains(tR[k])) touchedRanges.add(range);
                }
            }
        ...
        retagOverlapRange : function (startDate, endDate, lane) {
            ...
            var events = this.findEventsInRange(start, end, lane);
            // 3) re-tag and render those events
        ...
        renderEvents : function (events, isLane) {
            ...
            events.map(function (event) {
                ...
                var win = cal._findEventWindow(event);
                if (win) {
                    win.event = event;
        ...
        sizeEventWindow : function (eventWin, forceRedraw) {
            ...
            if (isTimeline) {
                ...
                var rowSize = this.body.getRowHeight(1),
                    ...
                    eHrs = event[cal.endDateField].getHours() == 0 ? 24
    ...
    //            eventWin.renderEvent(eTop, eLeft, eWidth, eHeight); //updateTitle();
            }
...
isc.DaySchedule.addProperties({
    ...
    initWidget : function () {
        ...
        if (cal.showDayLanes && !this._isWeek && cal.alternateLaneStyles) {
    ...
    rebuildFields : function () {
        ...
        if (cal.showDayLanes && cal.lanes && !this._isWeek) {
            fields[0].frozen = true;
            var scaffolding = isc.DaySchedule._getEventScaffolding(cal.getMinutesPerRow(this), cal.chosenDate);
            for (var i=0; i<cal.lanes.length; i++) {
                var lane = cal.lanes[i],
                    laneName = lane[cal.laneNameField] || lane.name,
                    props = { name: laneName, align: "center", canReorder: cal.canReorderLanes }
                ;
                props[cal.laneNameField] = laneName;
                fields.add(isc.addProperties(props, lane));
            ...
            fields[0].frozen = true;
            fields.add({name: "day1", align: "center"});
            if (this._isWeek) {
                var numDays = 8;
    ...
    getCellDate : function (rowNum, colNum) {
        var cal = this.getCalendar(),
            result;
        if (!this.isWeekView()) {
            result = cal.chosenDate.duplicate();
        } else {
            var fld = this.body.fields[colNum];
            if (!fld || !fld._yearNum) return;
            // for weekView, date props are stored on the field objects
            result = isc.Date.createLogicalDate(fld._yearNum, fld._monthNum, fld._dateNum);
        }
        // consolidate logic for dealing with minutes here
        if (result) {
            // each row represents calendar.getMinutesPerRow() minutes
            var minsPerRow = cal.getMinutesPerRow(this),
                rowsPerHour = cal.getRowsPerHour(this),
                hours = Math.floor(rowNum / rowsPerHour),
                extraRows = rowNum - (rowsPerHour * hours),
                mins = extraRows * minsPerRow
            ;
        ...
        if (this.isWeekView()) {
            ...
            for (var i=0; i < fields.length; i++) {
                var field = fields[i],
                    ...
                    date = cal.chosenDate // getDateFromCol(i)
                ;
                if (isc.isA.Date(date)) {
                    var time = isc.Time.parseInput(cal.getWorkdayStart(date, lane));
                    if (isc.Date.compareDates(result.start, time) < 0) {
                        result.start = time;
                    }
                    var time = isc.Time.parseInput(cal.getWorkdayEnd(date, lane));
                    if (isc.Date.compareDates(result.end, time) > 0) {
                        result.end = time;
                    }
                }
    ...
    refreshEvents : function () {
        ...
        this.clearEvents();
        var startDate, endDate, cal = this.getCalendar();
        if (this.isWeekView()) {
        ...
        var startFind = isc.timeStamp();
        var events = cal._getEventsInRange(startDate, endDate, this);
        var endFind = isc.timeStamp();

        //isc.logWarn("cal._getEventsInRange() call took " + ((endFind - startFind) / 1000));

        var startTag = isc.timeStamp();
        this.tagDataForOverlap(events);
        var endTag = isc.timeStamp();

        //isc.logWarn("this.tagDataForOverlap() call took " + ((endTag - startTag) / 1000));
    ...
    addEvent : function (event, retag) {
        ...
        var cal = this.getCalendar(),
            win = cal._getNewEventWindow(event, this),
            hideWindow = false
        ;
        ...
        if (!this._isWeek && cal.showDayLanes) {
        ...
        if (!hideWindow) {
            ...
            // event, combine the event-list from each of them, add the new event that list,
            // remove the existing ranges and then retag the events
            if (retag) {
...
isc.EventWindow.addProperties({
    ...
    checkForOverlap : function (event, startDate, endDate, lane) {
        ...
        var overlappingEvents = this._parentView.findOverlappingEvents(event, overlapTest);
...
isc.TimelineView.addProperties({
    ...
    _refreshData : function () {
        ...
        if (this.creator.dataSource && isc.ResultSet && isc.isA.ResultSet(this.creator.data)) {
            ...
            this.creator.fetchData(this.creator.getNewCriteria());
        } else {
    ...
    */
    ...
    refreshEvents : function () {
        ...
        // unnecessary - called in tagDataForOverlap()
        // this.forceDataSort();
     ...
     //isc.logWarn('ebtView refreshEvents:' + events.getLength());
        ...
        var laneNames = isc.getKeys(cal.getLaneMap()); //events.getProperty(cal.laneNameField).getUniqueItems();
    ...
    getVisibleEvents : function () {
        ...
        for (var i = 0; i < events.getLength(); i++) {
            ...
            var rangeObj = {};
            rangeObj[cal.laneNameField] = laneName;
            // if we're not showing lead-trail lines use start-endDate fields instead to
            // determine overlap
            if (event[cal.leadingDateField] && event[cal.trailingDateField]) {
           ...
           // isc.logWarn('eventRowIndex:' + eventRowIndex);
            ...
            var canEditLane = cal.canEditEventLane(event);
            if (this.eventsOverlap(rangeObj, event, canEditLane) && rowRange[0] <= eventRowIndex
