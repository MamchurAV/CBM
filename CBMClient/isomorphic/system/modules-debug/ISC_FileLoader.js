...
isc.version = "v14.1p_2026-08-14/LGPL Deployment";
isc.versionNumber = "v14.1p_2026-08-14";
isc.buildDate = "2026-08-14";
isc.expirationDate = "";

isc.scVersion = "14.1p";
isc.scVersionNumber = "14.1";
isc.sgwtVersion = "14.1p";
isc.sgwtVersionNumber = "14.1";
...
// Whether this browser exposes a SpeechRecognition implementation (the API class exists on
// <code>window</code>).  This indicates that browser-provided speech-to-text <i>may</i> be
// available for features such as +link{class:VoiceAssist}, but does not guarantee that
// recognition sessions can actually be started - see +link{Browser.allowsSpeechRecognition}
// for runtime viability.
// <p>
// This flag is set synchronously at page load.  A value of <code>false</code> means the
// browser does not implement SpeechRecognition at all (for example, Firefox).
// @visibility external
//<
...
// Whether the browser allows speech recognition sessions to succeed - that is, whether the
// SpeechRecognition implementation can contact the vendor-operated speech-to-text service it
// depends on.
// <p>
// Some browsers (notably Brave) expose the SpeechRecognition API but block the underlying
// service, causing an immediate "network" error when a session is started.  There is no way
// to detect this without actually starting a session, which requires microphone access,
// displays browser recording-chrome, and may trigger a permissions dialog.
// <p>
// This flag starts as <code>null</code> and is set to <code>true</code> or
// <code>false</code> either by an explicit call to
// +link{Browser.checkSpeechRecognition()}, or automatically by +link{class:VoiceAssist} the
// first time a real recording is attempted (in which case VoiceAssist handles the failure
// gracefully without requiring a separate probe).
// @see Browser.checkSpeechRecognition()
// @visibility external
//<
isc.Browser.allowsSpeechRecognition = null;

//> @classAttr Browser.micAvailable (Boolean : null : RA)
// Whether at least one audio input device (microphone) is available to the browser.  This
// reflects device presence only and does not imply that the site has
...
// <p>
// This is populated asynchronously at page load via <code>enumerateDevices()</code>.  To
// refresh it later (for example, after a device is plugged in), call
// +link{Browser.checkMicStatus()}.
// @see Browser.checkMicStatus()
// @visibility external
//<
...
// (origin).  Possible values are <code>"granted"</code>, <code>"denied"</code>, or
// <code>"prompt"</code>, consistent with the Permissions API.
// <p>
// <code>"prompt"</code> indicates that the user has not yet made a decision for this site
// and the browser will request permission when microphone access is first attempted,
// typically in response to a user interaction.
// <p>
// Not all browsers support querying microphone permission state in advance; in such cases
// this value may remain <code>null</code> until an access attempt is made.  To refresh it,
// call +link{Browser.checkMicStatus()}.
// @see Browser.checkMicStatus()
// @visibility external
//<
isc.Browser.micPermission = null;

// non-intrusive mic + permission check used at startup and by checkMicStatus()
isc.Browser._checkMicrophoneStatus = function () {
    if (
    ...
    return navigator.mediaDevices.enumerateDevices()
        .then(function (devices) {
            ...
            if (!hasMic) return { exists: false, permission: null };

            // query permission state without triggering a dialog
            return navigator.permissions
                .query({ name: 'microphone' })
                .then(function (result) {
                    return { exists: true, permission: result.state };
                })
                .catch(function () {
...
// run the non-intrusive check at startup
isc.Browser._checkMicrophoneStatus()
...
// Refreshes +link{Browser.micAvailable} and +link{Browser.micPermission} by re-querying the
// browser's device list and permission state.  This is non-intrusive: it does not trigger a
// permissions dialog or activate the microphone.
// <p>
// This check runs automatically at page load.  Call it explicitly to pick up changes that
// occurred after load, such as a microphone being plugged in or the user changing site
// permissions via browser settings.
// <p>
// The callback receives a single argument, an object with two properties:
// <ul>
// <li><code>micAvailable</code> (Boolean) - whether a mic is present</li>
// <li><code>micPermission</code> (String) - the permission state, or null if the Permissions
//     API is unavailable</li>
// </ul>
//
// @param [callback] (Function) called when the check completes
// @see Browser.micAvailable
// @see Browser.micPermission
// @see Browser.checkSpeechRecognition()
// @visibility external
//<
isc.Browser.checkMicStatus = function (callback) {
    return isc.Browser._checkMicrophoneStatus()
        .then(function (status) {
            isc.Browser.micAvailable = status.exists;
            isc.Browser.micPermission = status.permission;
            var result = {
                micAvailable: status.exists,
                micPermission: status.permission
            };
            if (callback) isc.Class.fireCallback(callback, "result", [result]);
            return result;
        })
        .catch(function () {
            isc.Browser.micAvailable = false;
...
// Tests whether the browser's SpeechRecognition implementation can successfully contact the
// vendor-operated speech-to-text service it depends on, and updates
// +link{Browser.allowsSpeechRecognition}, +link{Browser.micAvailable}, and
// +link{Browser.micPermission} accordingly.
// <p>
// <b>This method is intrusive</b>: it calls <code>getUserMedia({audio:true})</code>, which
// activates the microphone, shows browser recording-chrome, and may trigger a permissions
// dialog if the user has not already granted microphone access.  It then starts a short
// throwaway SpeechRecognition session to detect browsers (such as Brave) that expose the
// API but block the underlying service.
// <p>
// +link{voiceAssist.setActive()} calls this method - whether invoked directly or triggered
// by a triple-tap - before activating.  If the probe fails, +link{class:VoiceAssist} shows
// an explanatory message and does not activate.  The result is cached, so subsequent calls
// skip the probe.  Developers may also call this method independently - for example, to
// decide whether to show a voice-input button in the UI.
// <p>
// The callback receives a single argument, an object with these properties:
// <ul>
// <li><code>mic</code> (Boolean) - whether mic access was granted</li>
// <li><code>speech</code> (Boolean) - whether the speech service responded (not
//     blocked)</li>
// <li><code>reason</code> (String) - a short token describing the outcome:
//     <code>"ok"</code>, <code>"denied"</code>, <code>"no-mic"</code>,
//     <code>"speech-blocked"</code>, <code>"speech-unsupported"</code>,
//     <code>"unsupported"</code>, or <code>"exception"</code></li>
// </ul>
//
// @param [callback] (Function) called when the check completes
// @param [delay] (Integer) milliseconds to wait for a "network" error before concluding
//        the service is reachable; default 1000
// @see Browser.allowsSpeechRecognition
// @see Browser.supportsSpeechRecognition
// @see Browser.checkMicStatus()
// @visibility external
//<
isc.Browser.checkSpeechRecognition = function (callback, delay) {
    if (delay == null) delay = 1000;

    // if no SpeechRecognition class, resolve immediately
    if (!isc.Browser.supportsSpeechRecognition) {
        var result = {
            mic: false, speech: false, reason: "speech-unsupported"
        };
        if (callback) {
            isc.Class.fireCallback(callback, "result", [result]);
        }
    ...
    return new Promise(function (resolve) {
        ...
        // called for both getUserMedia paths when mic access fails - inspects the
        // error to distinguish "no mic hardware" from "permission denied"
        var onMicDenied = function (err) {
            var reason;
            var errName = err && err.name;
            if (errName === "NotFoundError" ||
                errName === "OverconstrainedError")
            {
                // no audio input device exists (e.g. Docker container, kiosk)
                isc.Browser.micAvailable = false;
                reason = "no-mic";
            } else {
                // NotAllowedError (user/policy denied), or any other failure
                isc.Browser.micPermission = "denied";
                reason = "denied";
            }
            var result = { mic: false, speech: false, reason: reason };
            if (callback) {
                isc.Class.fireCallback(callback, "result", [result]);
            }
            resolve(result);
        ...
        // called when mic access is granted - starts a throwaway SpeechRecognition session
        // to test whether the service is reachable
        var onMicGranted = function (stream) {
            // stop the stream immediately - we only needed permission
            if (stream && stream.getTracks) {
                stream.getTracks().forEach(function (t) { t.stop(); });
            }
            isc.Browser.micAvailable = true;
            isc.Browser.micPermission = "granted";
            ...
            // cleanup helper - stops recognition and resolves once
            var cleanup = function (speechOk) {
                if (resolved) return;
                resolved = true;
                isc.Timer.clearTimeout(successTimer);
                try { recognition.stop(); } catch (e) {}

                isc.Browser.allowsSpeechRecognition = speechOk;
                var result = {
                    mic: true,
                    speech: speechOk,
                    reason: speechOk ? "ok" : "speech-blocked"
                };
                if (callback) {
                    isc.Class.fireCallback(callback, "result", [result]);
                }
            ...
            // an immediate "network" error means the browser blocks the speech-to-text
            // service (e.g. Brave)
            recognition.onerror = function (event) {
                if (event.error == "network") cleanup(false);
            };

            // if speech actually starts, the service is working
            recognition.onspeechstart = function () {
                cleanup(true);
            };

            // if no error arrives within the delay, assume success
            var successTimer = isc.Timer.setTimeout(function () {
                cleanup(true);
            }, delay);
            ...
            try {
                ...
                resolved = true;
                isc.Timer.clearTimeout(successTimer);
                isc.Browser.allowsSpeechRecognition = false;
                var result = {
                    mic: true, speech: false, reason: "exception"
                };
                if (callback) {
                    isc.Class.fireCallback(callback, "result", [result]);
                }
        ...
        // legacy getUserMedia callback API
        var legacyGUM = navigator.getUserMedia ||
            navigator.webkitGetUserMedia ||
            navigator.mozGetUserMedia ||
            navigator.msGetUserMedia;
        if (legacyGUM) {
            legacyGUM.call(navigator, { audio: true }, onMicGranted, onMicDenied);
            return;
        }

        // no getUserMedia at all
        var result = {
            mic: false, speech: false, reason: "unsupported"
        };
        if (callback) {
            isc.Class.fireCallback(callback, "result", [result]);
        }
...
isc.addGlobal("Params", function (frame, blankValue) {
    ...
    if (questionIndex != -1) {
        ...
        for (var i = 0, param, equalIndex, prop; i < params.length; i++) {
            ...
            this[prop] = equalIndex >= 0 ? decodeURIComponent(param.substring(equalIndex+1)) :
                                           blankValue;
        }
    }
});
